 ...
 *
 * PrecircumfixPermutation = [ PermSign ] , Begin, { PermItem} , End ;
 * PermSign = Plus | PlusPlus | Minus ;
 * PermItem = [PermSign] , Face{1,3} , [ Number ] ;
...
public class ScriptParser0 {
    private Notation notation;

    public ScriptParser0(Notation notation) {
    ...
    private void parsePermutation(Tokenizer tt, Node parent, String token, Symbol symbol) throws ParseException {
        ...
        if (syntax == Syntax.PRECIRCUMFIX) {
            sign = this.parsePermutationSign(tt, parent);
        }
        if (tt.nextToken() != Tokenizer.TT_KEYWORD ||
     ...
     */
    Node parsePermutationItem(Tokenizer t, PermutationNode parent, Syntax syntax) throws ParseException {
