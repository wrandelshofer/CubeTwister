...
public class Solver {
     ...
     * by the search.
     */
    public static synchronized void initializeTables(@Nullable ProgressObserver pm) {
    ...
    private void rememberSolution() {
        ...
        for (i = 0; i < solutionLength1; i++) {
            ...
            solution.add(new MoveNode(Cube.symbolOfMove(translatedMove)));

            buf.append(Cube.nameOfMove(translatedMove));
            buf.append(' ');
        }

        solution.add(new NOPNode());
        buf.append(". ");
    ...
    private int solve2(@Nonnull KociembaCube cube) {
        int iteration = 1;
        int result = NOT_FOUND;
