...
public abstract class AbstractCube6Geom3D extends AbstractCubeGeom3D {
    ...
    protected void initTransforms() {
        ...
        for (int i = 0; i < cornerCount; i++) {
            identityTransforms[cornerOffset + i].translate(PART_LENGTH * -2.5f, PART_LENGTH * 2.5f, PART_LENGTH * 2.5f);
        }
        // urf
        ...
        for (int i = 0; i < edgeCount; i++) {
            switch (i % 24) {
                ...
                case 10:
                case 11:
                    identityTransforms[edgeOffset + i].translate(PART_LENGTH * 0.5f + PART_LENGTH * (i / 24), PART_LENGTH * 2.5f, PART_LENGTH * 2.5f);
                    break;
                default:
                    identityTransforms[edgeOffset + i].translate(PART_LENGTH * -0.5f - PART_LENGTH * (i / 24), PART_LENGTH * 2.5f, PART_LENGTH * 2.5f);
                    break;
        ...
        for (int i = 0; i < sideCount; i++) {
            switch (i / 6) {
                ...
                case 0:
                    identityTransforms[sideOffset + i].translate(PART_LENGTH * -0.5f, PART_LENGTH * -0.5f, PART_LENGTH * 2.5f);
                    break;
                case 1:
                    identityTransforms[sideOffset + i].translate(PART_LENGTH * -0.5f, PART_LENGTH * 0.5f, PART_LENGTH * 2.5f);
                    break;
                ...
                case 4:
                    identityTransforms[sideOffset + i].translate(PART_LENGTH * -1.5f, PART_LENGTH * -1.5f, PART_LENGTH * 2.5f);
                    break;
                case 5:
                    identityTransforms[sideOffset + i].translate(PART_LENGTH * -1.5f, PART_LENGTH * 1.5f, PART_LENGTH * 2.5f);
                    break;
                ...
                case 8:
                    identityTransforms[sideOffset + i].translate(PART_LENGTH * -0.5f, PART_LENGTH * -1.5f, PART_LENGTH * 2.5f);
                    break;
                case 9:
                    identityTransforms[sideOffset + i].translate(PART_LENGTH * -1.5f, PART_LENGTH * 0.5f, PART_LENGTH * 2.5f);
                    break;
                case 10:
                    identityTransforms[sideOffset + i].translate(PART_LENGTH * 0.5f, PART_LENGTH * 1.5f, PART_LENGTH * 2.5f);
                    break;
    ...
    public void cubeTwisted(@Nonnull CubeEvent evt) {
        ...
        switch (evt.getAxis()) {
            case 0:
                // Reduce number of faces to be drawn to optimize speed
                for (int i = 0; i < sideCount / 6; i++) {
                    shapes[sideOffset + cube.getSideAt(i * 6 + 1)].setReduced(true);
                    shapes[sideOffset + cube.getSideAt(i * 6 + 4)].setReduced(true);
                }
                break;
            case 1:
                // Reduce number of faces to be drawn to optimize speed
                for (int i = 0; i < sideCount / 6; i++) {
                    shapes[sideOffset + cube.getSideAt(i * 6 + 2)].setReduced(true);
                    shapes[sideOffset + cube.getSideAt(i * 6 + 5)].setReduced(true);
                }
                break;
            case 2:
                // Reduce number of faces to be drawn to optimize speed
                for (int i = 0; i < sideCount / 6; i++) {
                    shapes[sideOffset + cube.getSideAt(i * 6 + 0)].setReduced(true);
                    shapes[sideOffset + cube.getSideAt(i * 6 + 3)].setReduced(true);
                }
                break;
        } // end switch
