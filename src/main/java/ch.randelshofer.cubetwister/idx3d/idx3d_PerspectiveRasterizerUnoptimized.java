...
public final class idx3d_PerspectiveRasterizerUnoptimized extends idx3d_Rasterizer {
    ...
    // Constructor
    public idx3d_PerspectiveRasterizerUnoptimized(@Nullable idx3d_RenderPipeline pipeline) {
    ...
    private void renderTrapezoid(@Nonnull Vertex p1, @Nonnull Vertex p2, @Nonnull Vertex p3, @Nonnull Vertex p4, int ymin, int ymax) {
        ...
        long p2yp1y = p2.y - p1.y;
        long p4yp3y = p4.y - p3.y;
        for (y = ymin; y < ymax; y++) {
            interpolate(p1, p2, ((long) (y - p1.y) << FPbits) / p2yp1y, pL);
            interpolate(p3, p4, ((long) (y - p3.y) << FPbits) / p4yp3y, pR);
            int xStart = max(0, (int) pL.x);
            int xEnd = min(width, (int) pR.x);
            renderLineT(xStart, xEnd);
            /*
             if (xStart > xEnd) { // This is an invariant - move it out of the loop!
             renderLineT0(xEnd, xStart);
             } else {
             renderLineT0(xStart, xEnd);
             }*/
        }
    }

    /** Renders a line using y, pL, pR without optimisation. */
    private void renderLineT(int xStart, int xEnd) {
        for (x = xStart; x < xEnd; x++) {
            long a = ((long) (x - pL.x) << FPbits) / (pR.x - pL.x);
            interpolate(pL, pR, a, pX);
            plot(pX);
        }
    }

    /**
     * Plots without optimization.
     */
    private final void plot(@Nonnull Vertex p) {
        pos = (y * width + x);
        z = (int) (p.z >> FPbits);
        tx = p.w == 0 ? 0 : (int) (p.u / p.w);
        ty = p.w == 0 ? 0 : (int) (p.v / p.w);
        if (z < zBuffer[pos]) {
            bkgrd = screen.p[pos];
