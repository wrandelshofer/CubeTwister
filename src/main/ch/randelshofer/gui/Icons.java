 ...
 * @version 2.0 2010-08-18 Rewritten.
...
public class Icons {
    ...
    public static Icon get(int index) {
        if (index==SCRIPT_CHECK) return getScriptCheckIcon();
        if (index==SCRIPT_CHECK_DISABLED) return getScriptCheckDisabledIcon();
        if (icons[index] == null) {
            String path="/org/monte/media/gui/images/"+names[index];
            URL url =  Icons.class.getResource(path);
            if (url==null) throw new InternalError("Icons icon not found:"+path);
